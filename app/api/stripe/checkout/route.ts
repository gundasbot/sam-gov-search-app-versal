...
function normalizeBilling(raw: any): BillingInterval | null {
  const v = String(raw || '').toLowerCase().trim()
  if (!v) return null
...
function priceFromEnv(plan: 'basic' | 'professional' | 'enterprise', billing: BillingInterval): string | null {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const customerId = await ensureCustomerId(email, name, user.stripeCustomerId || null)
    if (!customerId) {
    ...
    const existingSub = await findActiveSubscription(customerId, user.stripeSubscriptionId || null)
