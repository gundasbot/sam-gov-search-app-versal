...
- recommendation (one sentence recommendation)`
    ...
    if (!isLoggedIn) {
      ...
      try {
        ...
        if (cached) {
          ...
          // Never show mock data — show empty state instead
        }
      } catch {
        // Never fall back to mock data for guests either
      }
  ...
  useEffect(() => {
    ...
    fetch('/api/account/preferences', { cache: 'no-store' })
      ...
      .then(prefs => {
        ...
        // Users can click "Update Preferences" to apply filters manually
      })
      .catch(() => {})
  }, [isLoggedIn])
    ...
    if (allOpportunities.length === 0 && !error) {
      ...
      const fetchInitialOpportunities = async () => {
        ...
        try {
          ...
          // Fetch broad set — preferences filter happens client-side via applyFilters
          // This ensures we always get a full page of results
          let url = '/api/sam/opportunities?limit=100&status=active';
  ...
  useEffect(() => {
    ...
    if (!isLoggedIn) {
      // Guests see live SAM.gov data (fetched weekly) or nothing — never mock
      if (allOpportunities.length === 0 && !guestDataLoading) {
        // Keep empty — guest fetch useEffect handles loading
      }
      if (!dataLoaded) {
        setDataLoaded(true);
        setLastUpdated('Just now');
        ...
        <div className="max-w-[1920px] mx-auto px-3 sm:px-6 lg:px-10 xl:px-12 py-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              {/* Update Preferences moved to welcome banner only */}
              <button
        ...
        <div ref={resultsRef} className="mb-2 flex items-center justify-between flex-wrap gap-2">
          {!isLoggedIn ? (
            ...
            <h3 className="text-xl font-bold text-white">
              ...
              {viewMode !== 'grid' && allOpportunities.length > 0 && (
                <> of <span className="text-cyan-400">{allOpportunities.length.toLocaleString()}</span> available</>
              )}
              <span className="text-slate-400 font-normal text-base">
                {viewMode === 'grid' ? ' opportunities with deadlines' : ' opportunities'}
              </span>
              {activeFilter && (
        ...
        <div className="mt-10 mb-24 text-center relative">
          <div className="mb-6 flex flex-col items-center gap-4">
            ...
            {isLoggedIn ? (
              ...
              <p className="text-sm text-slate-500">
                Showing <span className="text-slate-300 font-semibold">{(viewMode === 'grid' ? boardFiltered.length : visibleOpportunities.length).toLocaleString()}</span>{' '}
              {viewMode === 'grid' ? 'opportunities with active deadlines' : 'opportunities'}c = 'force-dynamic';
          ...
          <div className="mb-6 flex flex-col items-center gap-4">
            ...
            {isLoggedIn && !hasMore && dataLoaded && (
              <div style={{
                ...
                <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                All {(viewMode === 'grid' ? boardFiltered.length : allOpportunities.length).toLocaleString()} opportunities loaded
