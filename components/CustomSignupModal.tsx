...
export default function CustomSignupModal({ isOpen, onClose, onSuccess }: CustomSignupModalProps) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    setGoogleError('')
    try {
      // Redirect-based OAuth: NextAuth handles the callback
      await signIn('google', {
        callbackUrl: '/dashboard',
        redirect: true,
      })
      // If redirect: true the browser navigates away; onSuccess is called on return via session
      onSuccess?.()
    } catch (err: any) {
          ...
          <div onClick={(e) => e.stopPropagation()} style={{
            ...
            <div style={{ padding: '1.75rem 2rem' }}>
              ...
              {[
                ...
                'You\'re automatically signed in & ready to go',
              ].map((text, i) => (
              ...
              {/* Plan badge */}
              <div style={{ marginTop: '1.25rem', padding: '0.875rem 1rem', background: '#f0fdf4', border: '2px solid #bbf7d0', borderRadius: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
              ...
              <p style={{ fontSize: '0.8125rem', color: '#94a3b8', textAlign: 'center', margin: '1rem 0 0.75rem', fontFamily: font }}>
                Didn't receive it? Check spam or{' '}
                <button onClick={() => setSignupComplete(false)} style={{ background: 'none', border: 'none', color: '#059669', cursor: 'pointer', fontWeight: 700, fontSize: '0.8125rem', fontFamily: font, padding: 0, textDecoration: 'underline' }}>
                  try a different email
                </button>
              </p>

              <button onClick={onClose} style={{ width: '100%', padding: '0.875rem', background: 'linear-gradient(135deg, #10b981, #059669)', color: '#ffffff', border: 'none', borderRadius: '0.75rem', fontSize: '1rem', fontWeight: 700, cursor: 'pointer', fontFamily: font, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', boxShadow: '0 4px 12px rgba(16,185,129,0.3)' }}>
                Got it — I'll check my email <ArrowRight style={{ width: '18px', height: '18px' }} />
              </button>

              {/* Verified already? Sign in with Google shortcut */}
              <div style={{ marginTop: '0.875rem', textAlign: 'center' }}>
                <p style={{ fontSize: '0.8125rem', color: '#94a3b8', margin: '0 0 0.625rem', fontFamily: font }}>
                  Already verified or prefer Google?
                </p>
                <button
                  type="button"
                  ...
                  style={{
                    ...
                    fontSize: '0.9375rem', fontWeight: 700, fontFamily: font,
                    opacity: googleLoading ? 0.7 : 1,
                  }}
                >
                  {googleLoading ? (
                    <Loader2 style={{ width: '1.125rem', height: '1.125rem', animation: 'spin 1s linear infinite', color: '#64748b' }} />
                  ) : (
